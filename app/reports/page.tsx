...
import { CalendarClock, Clock3, Download, FileDown, FileSpreadsheet, FileText, ListFilter } from "lucide-react";
...
const periods: ReportPeriod[] = ["Harian", "Mingguan", "Bulanan", "Kustom"];
...
export default function ReportsPage() {
  ...
  const [rangeFrom, setRangeFrom] = useState(() => todayDate(timeZone));
  const [rangeTo, setRangeTo] = useState(() => todayDate(timeZone));
  ...
  const report = useMemo(() => buildReportExportModel({ tasks, activities, selectedDate, period, rangeFrom, rangeTo, timeZone }), [activities, period, rangeFrom, rangeTo, selectedDate, tasks, timeZone]);
  const isCustomPeriod = period === "Kustom";
  const rangeLabel = report.rangeFrom === report.rangeTo
    ? formatDate(report.rangeFrom, language, timeZone)
    : `${formatDate(report.rangeFrom, language, timeZone)} - ${formatDate(report.rangeTo, language, timeZone)}`;

  const text = {
    eyebrow: language === "id" ? "Laporan" : "Reports",
    title: language === "id" ? "Laporan Produktivitas" : "Productivity Reports",
    description: language === "id" ? "Ringkasan harian, mingguan, bulanan, dan rentang kustom dari data pribadi." : "Daily, weekly, monthly, and custom-range summaries of your personal data.",
    referenceDate: language === "id" ? "Tanggal acuan" : "Reference date",
    rangeFrom: language === "id" ? "Dari" : "From",
    rangeTo: language === "id" ? "Sampai" : "To",
    ...
    summary: language === "id" ? `Ringkasan ${isCustomPeriod ? rangeLabel : report.periodLabel}` : isCustomPeriod ? `Summary ${rangeLabel}` : `${tPeriod(period, language)} Summary`,
    highlights: language === "id" ? `Sorotan ${isCustomPeriod ? rangeLabel : report.periodLabel}` : isCustomPeriod ? `Highlights ${rangeLabel}` : `${tPeriod(period, language)} Highlights`,
    reportScope: isCustomPeriod
      ? language === "id" ? `Pada rentang ${rangeLabel}, ada ` : `For ${rangeLabel}, there are `
      : language === "id" ? `Pada ${report.periodLabel.toLowerCase()} dengan tanggal acuan ${formatDate(selectedDate, language, timeZone)}, ada ` : `For ${tPeriod(period, language).toLowerCase()} with reference date ${formatDate(selectedDate, language, timeZone)}, there are `,
    showControls: language === "id" ? "Tampilkan kontrol" : "Show controls",
    hideControls: language === "id" ? "Sembunyikan kontrol" : "Hide controls",
    showCharts: language === "id" ? "Tampilkan grafik" : "Show charts",
    hideCharts: language === "id" ? "Sembunyikan grafik" : "Hide charts"
  };

  function latestReport() {
    return buildReportExportModel({ tasks, activities, selectedDate, period, rangeFrom, rangeTo, generatedAt: getCurrentTimestampInTimeZone(timeZone), timeZone });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-5">
        <div className="flex items-center justify-between gap-3 md:hidden">
          <p className="text-sm font-semibold text-slate-950 dark:text-slate-50">{text.reportType}</p>
          <button type="button" onClick={() => setControlsExpanded((current) => !current)} className="inline-flex items-center gap-2 rounded border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"><ListFilter className="h-4 w-4" />{controlsExpanded ? text.hideControls : text.showControls}</button>
        </div>
        <div className={(controlsExpanded ? "grid" : "hidden md:grid") + " mt-4 gap-3 md:mt-0 md:grid-cols-2 xl:grid-cols-3"}>
          <label className="space-y-1"><span className="text-sm font-medium text-slate-700 dark:text-slate-200">{text.reportType}</span><select value={period} onChange={(event) => setPeriod(event.target.value as ReportPeriod)} className={getFieldClassName({ filled: Boolean(period) })}>{periods.map((item) => <option key={item} value={item}>{tPeriod(item, language)}</option>)}</select></label>
          {isCustomPeriod ? (
            <>
              <label className="space-y-1"><span className="text-sm font-medium text-slate-700 dark:text-slate-200">{text.rangeFrom}</span><input type="date" value={rangeFrom} onChange={(event) => setRangeFrom(event.target.value)} className={getFieldClassName({ filled: Boolean(rangeFrom) })} /></label>
              <label className="space-y-1"><span className="text-sm font-medium text-slate-700 dark:text-slate-200">{text.rangeTo}</span><input type="date" value={rangeTo} onChange={(event) => setRangeTo(event.target.value)} className={getFieldClassName({ filled: Boolean(rangeTo) })} /></label>
            </>
          ) : (
            <label className="space-y-1"><span className="text-sm font-medium text-slate-700 dark:text-slate-200">{text.referenceDate}</span><input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className={getFieldClassName({ filled: Boolean(selectedDate) })} /></label>
          )}
          <label className="space-y-1"><span className="text-sm font-medium text-slate-700 dark:text-slate-200">{text.pdfMode}</span><select value={pdfMode} onChange={(event) => setPdfMode(event.target.value as ReportPdfMode)} className={getFieldClassName({ filled: Boolean(pdfMode) })}>{pdfModes.map((item) => <option key={item} value={item}>{tReportPdfMode(item, language)}</option>)}</select></label>
        </div>
        <div className={(controlsExpanded ? "grid" : "hidden md:grid") + " mt-4 gap-2 sm:grid-cols-3"}>
      ...
      <button type="button" onClick={() => setChartsExpanded((current) => !current)} className="inline-flex w-full items-center justify-center gap-2 rounded border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 md:hidden"><ListFilter className="h-4 w-4" />{chartsExpanded ? text.hideCharts : text.showCharts}</button>

      <section className={(chartsExpanded ? "grid" : "hidden md:grid") + " gap-4 xl:grid-cols-2"}>
